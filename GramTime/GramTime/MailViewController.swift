...
class MailViewController: UIViewController, MFMailComposeViewControllerDelegate {
    ...
    func configuredMailComposeViewController() -> MFMailComposeViewController? {
        ...
        mailComposerVC.setSubject("SRVHRS - W\(report.weekNumber) - \(user.fullName) - \(report.projectNo) - \(calculateTravelingTime())")
    ...
    func calculateTravelingTime() -> String {
        let thisYear = time.year(forDate: report.mondayInWeek)
        let thisReportId = report.reportID
        let allUserReposts = realm.objects(WeekReport.self).filter("inspectorNo == %@", user.inspectorNumber)
            .filter { report -> Bool in
                report.officeReportWasSent || report.reportID == thisReportId
        }
        let sumOfTravelingInThisYear = allUserReposts.reduce(Double(0)) { (result, report) -> Double in
            let travelingTimeFromResult = report.travelingTime(inYear: thisYear)
            return result + travelingTimeFromResult
        }
        let sumInDays = sumOfTravelingInThisYear/(24*3600)
        return String(sumInDays.rounded(to: 1))
    }
}
